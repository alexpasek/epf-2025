...
export function garageMetadata(city) {
  ...
  return { title, description, alternates: { canonical: path }, robots: { index: true, follow: true }, openGraph: { title, description, url: path, type: "website", images: [{ url: hero, width: 1536, height: 1024, alt: heroAlt }] } };
}
function Actions() {
...
export default function GarageDrywallRepairPage({ city }) {
  ...
  return <main className="bg-slate-50 text-slate-900">
    ...
    <div className="container mx-auto max-w-6xl space-y-10 px-4 py-10 md:space-y-14">
      ...
      {coverage && <section id="neighbourhoods" className="scroll-mt-24 rounded-2xl border border-slate-200 bg-white p-6 md:p-9"><h2 className="text-2xl font-bold md:text-3xl">Garage drywall repair across {city.name} neighbourhoods</h2><p className="mt-5 max-w-4xl leading-8">{coverage.intro}</p><ul className="mt-5 grid grid-cols-1 gap-x-6 gap-y-2 text-slate-700 sm:grid-cols-2 lg:grid-cols-3">{coverage.names.map(name=><li key={name}>{name}</li>)}</ul><p className="mt-5 max-w-4xl leading-8">{coverage.planning}</p><p className="mt-4 leading-7">Your neighbourhood does not need to appear by name to request service. <Link href="/quote/" className="font-semibold text-blue-700 underline">Request a {city.name} garage repair estimate</Link> with your location and the areas needing attention.</p></section>}
